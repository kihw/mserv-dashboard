...
export default class PluginManager {
    ...
    instantiatePlugin(pluginConfig) {
      ...
      class Plugin {
        ...
        // Méthodes spécifiques définies dans la configuration
        ...this.createPluginMethods(pluginConfig);
      }
    ...
    /**
     * Crée les méthodes du plugin à partir de sa configuration
     * @param {Object} pluginConfig - Configuration du plugin
     * @returns {Object} Méthodes du plugin
     */
    createPluginMethods(pluginConfig) {
      const methods = {};
    ...
    triggerHook(hookName, data) {
      ...
      hooksForName.forEach(hookConfig => {
        try {
          hookConfig.handler.call(
            this.plugins.get(hookConfig.pluginId), 
            data
          );
        } catch (error) {
    ...
    savePluginConfigs() {
      try {
        const pluginConfigs = Array.from(this.plugins.values())
          .map(plugin => ({
            id: plugin.id,
            name: plugin.name,
